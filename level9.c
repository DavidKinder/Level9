...
* Additional code by Alan Staniforth
*
...
void checknumber(void)
{
	...
	{
		/* MS-DOS Scapeghost calls the input routine for a number before
		 * making any driver calls that would identify it as a V4 game,
		 * so here the code relies on the intial code scan noticing V4
		 * driver calls.
		 */
		if (L9GameType==L9_V4 || (L9GameType==L9_V3 && DriverV4Calls))
		{
			*list9ptr=1;
...
{
	l9textmode = *codeptr++;
	os_graphics(l9textmode && picturedata);
