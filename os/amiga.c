/* Amiga routines for Level 9 interpreter */
...
char Version[] = "$VER:Level9 2.0 (13.10.96)";
...
void
  screen_ratio (struct Screen *screen)
{
  struct TagItem vti[] =
...
void
  rect (long xMin, long yMin, long xMax, long yMax, unsigned long pen)
{
  unsigned long saved_pen;
...
int
  get_key (UWORD * qualifier_addr, int c, int wait)
{
...
void
  redraw_line (unsigned char *buffer, int *pos, int max_size)
{
  char *current_buffer;
...
void
  cursor_left (unsigned char *buffer, int *pos)
{
  while (*pos > 0)
  {
    (*pos)--;
...
void
  cursor_right (unsigned char *buffer, int *pos)
{
...
int
  fit_text (unsigned char *pointer, int new)
{
  int a, b;
...
void
  into_buffer (unsigned char *buffer, unsigned char *new, int *pos, int max_size)
{
  if (new == 0)
    return;
  cursor (*(buffer + *pos));
...
void
  filereq (struct FileRequester *freq, char *buffer, char *title, ULONG save)
{
