...
void os_drawline(int x1, int y1, int x2, int y2, int colour1, int colour2)
{
  if (top_y > 0)
  {
    ...
    {
      int pic_width;
      GetPictureSize(&pic_width,NULL);

      x1 *= ((double)gfx_width) / 320.0;
      y1 *= ((double)gfx_height) / 96.0;
      x2 *= ((double)gfx_width) / 320.0;
      y2 *= ((double)gfx_height) / 96.0;
...
void os_fill(int x, int y, int colour1, int colour2)
{
  if (top_y > 0)
  {
    ...
    if (high_res_pics)
    {
      ...
      x *= ((double)gfx_width) / 320.0;
      y *= ((double)gfx_height) / 96.0;
